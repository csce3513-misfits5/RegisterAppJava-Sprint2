...
public class DummyTransactionCreateCommand implements VoidCommandInterface {
	@Override
	public void execute() { //creates a transaction
		long transactionTotal = 0L; //keeps running total of transaction
		final List<TransactionEntryEntity> transactionEntryEntities = new LinkedList<>(); //Linked list of transaction entries

		for (ProductEntity productEntity : this.productRepository.findAll()) {
			int purchasedQuantity = ThreadLocalRandom.current().nextInt(1, 11); //Currently generating a random num for purchased quantity
	...
	// Helper methods
	@Transactional
	private void createDummyTransaction( //execute needs to be called first to update transactionTotal
